...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    if (fileParams.parentId !== 0 && !Utils.isValidId(fileParams.parentId)) { return response.status(400).send({ error: 'Parent not found' }); }
  ...
  static async getShow(request, response) {
    ...
    if (!Utils.isValidId(fileId) || !Utils.isValidId(userId)) { return response.status(404).send({ error: 'Not found' }); }
  ...
  static async getIndex(request, response) {
    ...
    if (parentId !== 0 && parentId !== '0') {
      if (!Utils.isValidId(parentId)) { return response.status(401).send({ error: 'Unauthorized' }); }
  ...
  static async getFile(request, response) {
    ...
    // Mongo Condition for Id
    if (!Utils.isValidId(fileId)) { return response.status(404).send({ error: 'Not found' }); }
