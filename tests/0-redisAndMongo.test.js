...
// redisClient
describe('Test for MongoDB and Redis', () => {
  describe('redis Client', () => {
    ...
    it('set key can be called without issue', async () => {
      expect(await redisClient.set('myKey', 10, 1)).to.equal(undefined);
    });
  ...
  describe('db Client', () => {
    ...
    it('shows number of user documents', async () => {
      ...
      await dbClient.usersCollection.insertOne({ name: 'Jones' });
    ...
    it('shows number of file documents', async () => {
      ...
      await dbClient.filesCollection.insertOne({ name: 'FileThree' });
