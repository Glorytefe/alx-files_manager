...
const fileUtils = {
  /**
   * checks that body is valid for creating file
  ...
  async publishUnpublish(request, setPublish) {
    ...
    if (!Utils.isValidId(fileId)) { return { error: 'Unauthorized', code: 401 }; }

    const { userId } = await userUtils.getUserIdAndKey(request);

    if (!Utils.isValidId(userId)) { return { error: 'Unauthorized', code: 401 }; }
